  ...
  const attachment = (file, idx) => {
    return (
      <Grid item key={idx} className={classes.contentGrid}>
        <Grid container alignItems="center" style={{ marginLeft: 16, width: `calc(100% - 50px)`, cursor: 'pointer' }} onClick={() => downloadFile(file.file_name_system)}>
  ...
  const noAttachments = () => {
    return (
      <Grid item xs={12} className={classes.contentGrid}>
        <Typography style={{ fontSize: 16, marginLeft: 16, color: '#001858' }}>No attachnents linked to this publication</Typography>
      </Grid>
    );
  };
