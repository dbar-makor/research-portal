...
import { Route, Switch, IndexRoute, useHistory } from "react-router-dom";
...
import Article from "./components/Researches/Article";
import AuthorsUsers from "./components/AdminScreens/AuthorsUsers";
import SalesUsersScreen from "./components/AdminScreens/SalesUsersScreen";
import AuthorsUsersScreen from "./components/AdminScreens/AuthorsUsersScreen";
...
function App() {
  ...
  const isAuthor = useSelector(
    (state) => state.auth.userContent?.type === "author"
  );
  const isSales = useSelector(
    (state) => state.auth.userContent?.type === "sales"
  );
  const isAdmin = useSelector(
    (state) => state.auth.userContent?.type === "admin"
  );
  const isMember = useSelector(
    (state) =>
      state.auth.userContent?.type === "client" ||
      state.auth.userContent?.type === "prospect"
  );
  ...
  return (
    <ThemeProvider theme={mainTheme}>
      ...
      {isAuthenticated &&
        (isMember ? (
          <FooterMember style={{ position: "absolute", bottom: 0 }} />
        ) : (
          <></>
        ))}
    </ThemeProvider>
