...
import { Box, Collapse, Grid, IconButton, Table, TableBody, TableCell, TableHead, TableRow, Typography } from '@material-ui/core';
...
function ContractRow(props) {
  ...
  const searchInvoice = (value) => {
    const debaunceSearch = setTimeout(() => {
      if (value !== '') {
        let filterInvoice = contract.invoices.filter((i) => i.id.slice(0, 6).includes(value) || i.id.slice(0, 6).toUpperCase().includes(value));
  ...
  const showInvoice = async (invoiceId) => {
    try {
      const res = await axios.get(`${BASE_URL}${END_POINT.INVOICE}/pdf/${invoiceId}`, { headers: { Accept: 'application/pdf' } });

      if (res.status === 200) {
        console.log("HERERERER")
        let pdfString = res.data.pdf;

        let byteCharacters = window.atob(pdfString);
    ...
    let url = `${BASE_URL}${END_POINT.INVOICE}/pdf/${invoiceId})}`;
  ...
  return (
    <>
      <TableRow style={{ backgroundColor: open ? '#f3f7ff' : '#ffffff' }}>
        <StyledTableCell>{`${format(new Date(contract.start_at), 'dd MMM, yyyy')}-${format(new Date(contract.end_at), 'dd MMM, yyy')}`}</StyledTableCell>
        ...
        <StyledTableCell style={{ textAlign: 'center' }}>{`${contract.currency.symbol}${calcYearlyCost(contract.amount, contract.periodicity)}`}</StyledTableCell>
        <StyledTableCell style={{ textAlign: 'center' }}>{contract.signed ? <Signed /> : <NotSigned />}</StyledTableCell>
        <StyledTableCell style={{ padding: '0px', textAlign: 'center' }}>
          <Grid container alignItems="center">
            <Grid item xs={12} alignItems="center">
              <PaymentButtonBar precentage={calcPaymentProgress(contract.invoices)} openInvoices={openInvoices} isOpen={open} contract={contract} clientName={clientName} />
            </Grid>
          </Grid>
        </StyledTableCell>
      </TableRow>
      <TableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0, borderBottom: 'none', backgroundColor: '#f9fafb' }} colSpan={12}>
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell className={classes.collapseTableHeaders} style={{ textAlign: 'right', paddingRight: '20px' }}>
                        Status
                      </TableCell>
                    </TableRow>
                  </TableHead>
                    ...
                    {filterInvoices &&
                      filterInvoices.map((invoice, idx) => (
                        <TableRow key={idx}>
                          ...
                          <TableCell style={{ fontWeight: 'bold' }}>{`${contract.currency.symbol}${invoice.amount.toLocaleString()}`}</TableCell>
                          <TableCell style={{ textAlign: 'center' }}>
                            <IconButton size="small" onClick={() => showInvoice(invoice.id)}>
                              <Paper />
                            </IconButton>
                          </TableCell>
                          <TableCell style={{ textAlign: 'right' }}>{invoice.status === 'approved' ? <Approved /> : invoice.status === 'pending' ? <Pending /> : <Rejected />}</TableCell>
                        </TableRow>
                      ))}
                  </TableBody>
                </Table>
