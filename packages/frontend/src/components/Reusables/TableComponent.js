  ...
  const tableHeadText = (col, index) => {
    return (
      <StyledTableCell key={index}>
        <Typography style={{ textTransform: 'capitalize', textAlign: col === 'status' || col === 'contract_status' ? 'center' : 'left' }}>{col.replaceAll('_', ' ')}</Typography>
      </StyledTableCell>
    );
  };
        ...
        <TableRow>
          {Object.keys(data[0]).map((col, index) => {
            if (type === 'client') {
              ...
              if (col !== 'id' && col !== 'members' && col !== 'logo' && col !== 'periodicity' && col !== 'contract_status') {
                return tableHeadText(col, index);
              }
            } else {
              if (col !== 'id' && col !== 'members' && col !== 'logo' && col !== 'periodicity' && col !== 'end_at') {
        ...
        {data.map((row, index) => (
          <StyledTableRow
            ...
            className={clsx(classes.tableRow, {
              [classes.selectedRow]: (chosenCompany && row.id === chosenCompany.id) || (chosenUser && row.id === chosenUser.id),
            })}
                ...
                if (key === 'start_at' || key === 'last_connected_at') {
                  return (
                    <StyledTableCell key={i} style={{ whiteSpace: 'nowrap' }} align="center">
                      {value ? format(new Date(value), 'HH:mm dd MMM, yyyy') : <Typography>&#8211;</Typography>}
                    </StyledTableCell>
                  );
                  ...
                  return (
                    <StyledTableCell key={i} align="center">
                      {<FiberManualRecordIcon style={{ fill: value ? '#00CA80' : '#FF3939', fontSize: '14px' }} />}
                    </StyledTableCell>
                  );
                  ...
                  return <StyledTableCell key={i}>{value.length > 12 ? `${value.slice(0, 12)}...` : value}</StyledTableCell>;
                } else {
