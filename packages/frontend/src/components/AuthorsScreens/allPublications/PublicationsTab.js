...
function truncateString(string, wordsNo) {
  let descrptionArr = string.split(' ')
  let descriptionLength = descrptionArr.length
  descrptionArr.splice(wordsNo)
  let newDescription = descrptionArr.join(' ')
...
function PublicationsTab({ publication, fetchPublications, fetchStatistics }) {
  ...
  return(<>
      ...
      <Grid container className={classes.card}>
        <Grid item xs={12} className={classes.upperHalf} style={{ backgroundImage: chooseImage(publication) ? `url(${chooseImage(publication)})` : 'none', backgroundColor: '#74b2f0' }}>
          ...
          {publication.type === 'dead' ? (
            <Grid className={classes.typeIndicator}>{publication.title_video ? <PlayArrowIcon className={classes.typeIcon} /> : <EmptyFile className={classes.typeIcon} />}</Grid>
          ) : (
            <></>
          )}
        </Grid>
    ...
    <DeleteAlert open={openAlert} handleClose={handleCloseAlert} itemName={publication.title} itemId={publication.id} itemCategory='Publication' deleteItem={deletePublication} />
  </>)
}
