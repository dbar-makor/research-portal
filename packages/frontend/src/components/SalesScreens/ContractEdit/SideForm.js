...
console.log("chosenCompany from contract", chosenCompany )
  ...
  const sendEmail = async () => {
    ...
    if(signerInputValue && activeSidebar){
      ...
      try {
        ...
        const res = await axios.put(`${BASE_URL}${END_POINT.CONTRACT}/${chosenContract.contract_id}`, {signer_user:contractSignerID});
  ...
  const presentPDFContract = async () => {
    try {
      const res = await axios.get(`${BASE_URL}${END_POINT.CONTRACT}/pdf/${chosenContract.contract_id}`,{ headers: {Accept: 'application/pdf'}});
          ...
          <Grid item xs={12}>
            <Grid container direction="column" spacing={3} className={classes.section}>
              <Grid item xs={11}  style={{ margin: '36px 0px 0px 16px' }}>
                <Typography className={classes.sectionTitle} style={activeSidebar ? {color: "#000"} : {}}>Updated Attachments:</Typography>
              </Grid>
              <Grid item xs={12}>
