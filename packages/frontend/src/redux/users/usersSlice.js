...
export const getUsersByTypeAsync = (offset, limit, search, type, status) => async (dispatch, getState) => {
  ...
  try {
    const res = await axios.get(`${BASE_URL}${END_POINT.USER}`, setParams(offset, limit, search, type, status))
    if (res.status === 200) {
...
export const { getUsersData, setLoading, deleteUser,setUsersByType, setUserProperty, setMetaData, setUserHasMore} =
usersSlice.actions
