...
Comment.createComment = async (payload, result) => {
  try {
    ...
    const [pubs] = await db_helper.get(publication_sql.get_publication_id_by_uuid(processed_data.publication_id))
    if (!pubs) {
      return result({ status: 500 })
    }
    processed_data.publication_id = pubs.id
    if (processed_data.comment_id) {
      const [comment] = await db_helper.get(publication_sql.get_comment_by_uuid(processed_data.comment_id))
      if (!comment) {
        return result({ status: 500 })
      }
...
Comment.getCommentsById = async (payload, result) => {
  try {
    ...
    const [pub] = await db_helper.get(publication_sql.get_publication_id_by_uuid(processed_data.publication_id))
    if (!pub) {
      return result({ status: 500 })
    }
            ...
            case 'bearerAuth':
              if (val.hasOwnProperty('user') && typeof val.user !== 'undefined') {
                if (val.user.hasOwnProperty('id') && typeof val.user.id !== 'undefined') {
                  ...
                  errors.push('[Error]: missing user id. must have user id to create comment!!!')
                }
              }
              break
            case 'commentCreate':
                  ...
                  switch (key_1) {
                    ...
                    case 'publication_id':
                      if (typeof val_1 === 'string') {
                        ...
                        errors.push(`[Error]: commentCreate.publication_id is not a number on create comment!!!`)
                      }
                      break
                    case 'comment_id':
                      if (typeof val_1 === 'string') {
                        ...
                        errors.push(`[Error]: commentCreate.comment_id is not a number on create comment!!!`)
                      }
                      break
                    case 'type':
          ...
          switch (key) {
            case 'id':
              ...
              if (typeof val === 'string') {
                ...
                errors.push(`[Error]: publication_id is not a string on get_by_publication_id comment!!!`)
              }
              break
            default:
              break
