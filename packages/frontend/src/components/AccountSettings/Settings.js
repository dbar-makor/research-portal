...
function Settings() {
  ...
  const getUserSettings = async () => {
    try {
      let res = await axios.get(`${BASE_URL}${END_POINT.SETTINGS}/notification`)
      if (res.status === 200) {
  ...
  const handleToggle =  (e, section, key) => {
    if (section === 'publications' || section === 'replies') {
      setUserSettings((prevState) => ({ ...prevState, settings: { ...prevState.settings, [section]: { ...prevState.settings[section], [key]: e.target.checked} } }))
            ...
            <Grid container>
              ...
              <Grid item xs={12}>
                <Grid container alignItems='center'>
                  <Grid item>
                    <AllNotificationSwitch checked={userSettings.is_active} onChange={(e) => handleToggle(e, 'is_active')} />
                  </Grid>
                  <Grid item>
            ...
            <Grid container>
              <Grid item xs={6}>
                <Grid container direction='column'>
                  <Grid item>
                    <Typography style={{ marginLeft: 10, color: '#868DA2', fontSize: 14 }}>Publications</Typography>
                  </Grid>
                    ...
                    Object.entries(userSettings.settings.publications).map(([key, value], idx) => {
                      return (
                        <Grid item key={idx}>
                          <Grid container alignItems='center'>
                            <Grid item>
                              <GreenSwitch disabled={userSettings.is_active ? false : true} checked={value} onChange={(e) => handleToggle(e, 'publications', key)} />
                            </Grid>
                            <Grid item>
                              <Typography style={{ textTransform: 'capitalize' }}>{key.replace('_', ' ')}</Typography>
                            </Grid>
                          </Grid>
                        </Grid>
                      )
              ...
              <Grid item xs={6}>
                <Grid container direction='column'>
                  <Grid item>
                    <Typography style={{ marginLeft: 10, color: '#868DA2', fontSize: 14 }}>Replies</Typography>
                  </Grid>
                    ...
                    Object.entries(userSettings.settings.replies).map(([key, value], idx) => {
                      return (
                        <Grid item key={idx}>
                          <Grid container alignItems='center'>
                            <Grid item>
                              <GreenSwitch disabled={userSettings.is_active ? false : true} checked={value} onChange={(e) => handleToggle(e, 'replies', key)} />
                            </Grid>
                            <Grid item>
                              <Typography style={{ textTransform: 'capitalize' }}>{key.replace('_', ' ')}</Typography>
                            </Grid>
