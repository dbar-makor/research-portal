...
import { GreenFilledButton, BinButton, EditButton, LinkButton, FilledButton, useStyles as useMainStyles, CompanyStatusSwitch, EditDoneButton } from '../../../styles/MainStyles';
...
import { selectChosenCompany, getChosenCompanyAsync, changeChosenCompany } from '../../../redux/companies/chosenCompanySlice';
...
function CompanyInfo() {
  ...
  useEffect(() => {
    if (chosenCompany && chosenCompany.prospect_trial && chosenCompany.prospect_trial.start_at && chosenCompany.prospect_trial.end_at) {
      setTrialPeriod(formatDistance(new Date(chosenCompany.prospect_trial.start_at), new Date(chosenCompany.prospect_trial.end_at)));
    }
  ...
  const currentContractProps = {
    ...
    contractStatus: chosenCompany && chosenCompany.contract_status !== undefined && chosenCompany.contract_status !== null && chosenCompany.contract_status,
  };
  ...
  const companyRegisteredProps = {
    ...
    sales_agent: chosenCompany && chosenCompany.company_registered && chosenCompany.company_registered.sales_agent,
  ...
  return chosenCompany ? (
    <Grid container className={classes.infoContainer}>
                    ...
                    <Grid container alignItems="center" justifyContent="space-between">
                      <Grid item xs={chosenCompany.name.length > 14 ? 8 : 6}>
                        {chosenCompany.isEditMode ? (
                          ...
                          <Typography className={classes.companyName}>{chosenCompany.name.length > 18 ? `${chosenCompany.name.slice(0, 18)}...` : chosenCompany.name}</Typography>
                        )}
                      </Grid>
                      <Grid item xs={1}></Grid>
                      <Grid item xs={3}>
                        <Grid container alignItems={chosenCompany.isEditMode ? 'flex-start' : 'center'} justifyContent={chosenCompany.isEditMode ? 'flex-end' : 'flex-start'}>
                          <Grid item xs={3}>
                            {chosenCompany.isEditMode ? (
                              <CompanyStatusSwitch checked={chosenCompany.status} onChange={(e) => updateCompanyField('status', e.target.checked)} />
                            ) : (
                          ...
                          <Grid item xs={9}>
                            <Typography
                              style={{ marginLeft: chosenCompany.isEditMode && '12px' }}
                              className={clsx({
                    ...
                    <Grid container alignItems="center" justifyContent="flex-end">
                      <Grid item style={{ marginRight: '16px' }}>
                        {chosenCompany.isEditMode ? (
                          ...
                          <EditButton
                            onClick={() => setChosenCompany({ ...chosenCompany, isEditMode: !chosenCompany.isEditMode })}
                      ...
                      <Grid item>
                        ...
                        <DeleteAlert open={openAlert} handleClose={handleCloseAlert} itemName={chosenCompany.name} itemId={chosenCompany.id} itemCategory="Company" deleteItem={deleteCompany} />
                      </Grid>
                    </Grid>
                  </Grid>
                </Grid>
      ...
      <Grid item xs={12}>
        <Grid container justifyContent="flex-end">
          <Grid item xs={6}>
            {chosenCompany.type === 'client' && (chosenCompany.contract_status || chosenCompany.last_contract) ? (
              <Grid container justifyContent="flex-end">
                <LinkButton className={classes.upgradeBtn} onClick={() => openContractDialong(chosenCompany.id)}>
                  Contracts & Invoices
                </LinkButton>
              </Grid>
            ) : null}
            <ContractsModal onClose={() => closeContractDialong()} isOpen={openDialog} client={chosenCompany} />
          </Grid>
          {!chosenCompany.contract_status ? (
            <Grid item xs={3}>
              <Grid container justifyContent="flex-end">
                {chosenCompany.type === 'prospect' ? (
                  <GreenFilledButton className={classes.upgradeBtn} onClick={() => upgradeToClient(chosenCompany.id)}>
                    Upgrade to Client
                  </GreenFilledButton>
                ) : (
