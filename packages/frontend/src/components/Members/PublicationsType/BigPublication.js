...
const BigPublication = ({ publication }) => {
  ...
  return (
    <Grid item xs={12} key={publication.id} style={{ padding: 14 }}>
      <Grid container>
        <Grid item xs={6}>
          <Link to={{ pathname: `article/${publication.id}`, state: {
      applied: true
    }}}>
            <Grid item xs={12} className={classes.sideHalf} style={{ backgroundImage: chooseImage(publication) ? `url(${chooseImage(publication)})` : 'none', backgroundColor: '#74b2f0' }}></Grid>
          </Link>
        </Grid>
