...
import { validateDeadPublication, validateEvent, validateEditedDeadPublication } from "../Reusables/validationFunctions";
...
function DeadArticle() {
  ...
  useEffect(() => {
    if (localForm) {
      tableRowsRefs.current = tableRowsRefs.current.slice(
        0,
        localForm.events.length
      );
      executeScroll();
    }
  }, [localForm.events]);
  ...
  useEffect(() => {
   ...
   if(chosenResearch){
    ...
    //  let categoriesIDs = chosenResearch.categories.map(category => category.id);
     let editedLocalForm = {...chosenResearch};
  ...
  useEffect(() => {
    if(location.state?.from === "prearticle"){
      ...
      let editedLocalForm = {...publication};
      delete editedLocalForm.created_at;
      delete editedLocalForm.name;
      delete editedLocalForm.updated_at;
  ...
  const handleCatsChange = (values) => {
    ...
    if(chosenResearch){
      validateEditedDeadPublication({categories : newCats}, errors, setErrors, setValidationResult, selectedValue)
    }else{
  ...
  const deleteItem = (index, category) => {
    if (category === "localCats") {
      ...
      setLocalForm({
        ...
        if(chosenResearch){
          validateEditedDeadPublication({categories : formCats}, errors, setErrors, setValidationResult, selectedValue)
        }else{
          validateDeadPublication({categories : formCats}, errors, setErrors, setValidationResult, selectedValue)
        }
    } else {
      const categoryCopy = [...localForm[category]];
    ...
    console.log("pdf", pdf)
      ...
      try {
        ...
        if (res.status === 200 && res.data.file) {
          ...
          if(chosenResearch){
            validateEditedDeadPublication({file_pdf : res.data.file}, errors, setErrors, setValidationResult, selectedValue)
          }else{
            validateDeadPublication({file_pdf : res.data.file}, errors, setErrors, setValidationResult, selectedValue)
          }
  ...
  const sendPublication = async (buttonMarker) => {
    ...
    if(buttonMarker === "done"){
      formToSend = {...formToSend,categories: categoriesForServer, tags: tagsForServer, status:"published"}
    }else if(buttonMarker === "save-draft"){
      console.log("i'm a draft")
      formToSend = {...formToSend,categories: categoriesForServer, tags: tagsForServer, status:"draft"}
      ...
      >
              ...
              <>
                      ...
                      <>
                       {shortify(localForm.file_pdf)}
                        <DeleteButton
                          disableRipple
                          onClick={() => {
                            setLocalForm(prev => ({...localForm, title_pdf: "", file_pdf: ""}))
                            if(chosenResearch){
                              validateEditedDeadPublication({file_pdf : localForm.title_pdf}, errors, setErrors, setValidationResult, selectedValue)
                            }else{
                              validateDeadPublication({file_pdf : localForm.title_pdf}, errors, setErrors, setValidationResult, selectedValue)
                            }
                ...
                >
                  ...
                  <AtricleTitleTextField
                    ...
                    onChange={(e) =>
                      handleChange(e.target.value, "title_video")
                    }
                    placeholder="Title"
                    inputProps={{
                      style: {
        ...
        <Grid container>
          <Grid item xs={12}>
            <Grid container>
              <Grid item xs={3}>
                <Button onClick={handleCancle} className={classes.cancelStyle}>cancel</Button>
              </Grid>
              <Grid item xs={3}>
