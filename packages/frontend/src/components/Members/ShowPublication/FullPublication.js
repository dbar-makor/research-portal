...
//in case user is member- pub is taken from the params
  const getPublication = async (id) => {
    setLoadingPub(true);
    if (id !== undefined) {
      let resp = await axios.get(`${BASE_URL}${END_POINT.PUBLICATION}/${id}`);
      try {
        if (resp.status === 200) {
          setLoadingPub(false);
          setChosenPublication(resp.data);
          console.log("resp.data", resp.data)
          let view = await axios.put(`${BASE_URL}${END_POINT.PUBLICATION}/view/${id}`);
        } else {
  ...
  return (
    <>
      <Grid container direction="row" justifyContent="center" style={{ paddingTop: 30, position: 'relative' }}>
        {loadingPub && (
          ...
          <>
            ...
            <Grid item xs={5}>
              <Grid container>
                ...
                <AuthorInfo auther={chosenPublication.author} lastDate={chosenPublication.updated_at ? chosenPublication.updated_at : chosenPublication.created_at} />
                {chosenPublication.type === 'live' ? (
                  ...
                  <PdfViewer pdf={{ title_pdf: chosenPublication.title_pdf, file_pdf: chosenPublication.file_pdf }} />
                ) : (
                  <VideoFrame video={{ title_video: chosenPublication.title_vide, link_video: transformVideoLink(chosenPublication.link_video) }} />
                )}
                {(chosenPublication.commments !== null || chosenPublication.commments !== undefined) && userType !== 'author' ? (
                  <Comments comments={chosenPublication.comments} pubId={chosenPublication.id} />
                ) : null}
                {(chosenPublication.categories !== null || chosenPublication.categories !== undefined) && userType !== 'author' ? (
                  <MorePublications categories={chosenPublication.categories} title={chosenPublication.title} />
                ) : null}
              </Grid>
            </Grid>
          </>
        ...
        {
          !loadingPub &&
          <IconButton style={{ backgroundColor: '#868DA2', height: 73, width: 73, position: 'absolute', bottom: '160px', right: '430px' }} onClick={() => window.scroll(0, 0)}>
