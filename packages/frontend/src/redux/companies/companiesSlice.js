  ...
  reducers: {
    setCompanyProperty: (state, action) => {
      if (action.payload.subProperty) {
        ...
        state.company[action.payload.mainProperty] =
          action.payload.updatedValue;
      }
    },
...
export const selectProspectCompanies = createSelector(
  [selectCompaniesData],
  (companies) => companies.filter((company) => company.status === "prospect")
);

export const selectPayingCompanies = createSelector(
  [selectCompaniesData],
  (companies) => companies.filter((company) => company.status === "client")
);
