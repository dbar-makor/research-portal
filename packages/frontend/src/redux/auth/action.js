...
export const login = (email, password) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      actionSnackBar.setSnackBar("success", "Successfully connected", 2000)
    );
  } catch (error) {
    console.log(error);
    if (error) {
      dispatch(
        actionSnackBar.setSnackBar(
          "error",
          "You don't have access to the platform",
          3000
        )
      );
    } else {
...
export const logout = () => async (dispatch) => {
  try {
    ...
    if (res.status === 200) {
      ...
      dispatch(
        actionSnackBar.setSnackBar("success", "Successfully disconnected", 2000)
      );
    }
  } catch (error) {
