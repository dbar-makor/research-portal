import { Divider, Grid, IconButton, makeStyles, Popover, TableCell, TableRow, Typography } from '@material-ui/core';
  ...
  return (
    <TableRow>
      <TableCell style={{ textAlign: 'center' }}>
        <FiberManualRecordIcon style={{ color: contract.status === false ? '#FF3939' : '#00CA80', fontSize: '12px' }} />
      </TableCell>
      <TableCell style={{ color: contract.signed === true ? '#00CA80' : '#FF3939' }}>{contract.signed === true ? 'Signed' : 'Unsigned'}</TableCell>
      <TableCell style={{ width: 215 }}>{`${format(new Date(contract.start_at), 'dd MMM , yyyy')} - ${format(new Date(contract.end_at), 'dd MMM, yyyy')}`}</TableCell>
      ...
      <TableCell style={{ width: 100, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
        {contract.company_country.length > 6 ? `${contract.company_country.slice(0, 6)}...` : contract.company_country}
      </TableCell>
      <TableCell style={{ textTransform: 'capitalize' }}>{contract.periodicity === 'fully' ? 'Yearly' : contract.periodicity}</TableCell>
      <TableCell style={{ textAlign: 'right' }}>{`${contract.currency.symbol}${contract.amount}`}</TableCell>
      <TableCell style={{ fontWeight: 'bold', textAlign: 'right' }}>{`${contract.currency.symbol}${calcYearlyCost(contract.amount, contract.periodicity)}`}</TableCell>
      <TableCell style={{ textAlign: 'center' }}>
          ...
          <Grid container direction="column" spacing={2} style={{ paddingTop: 10 }}>
            <Grid item xs={11} style={{ marginLeft: '16px' }}>
              <Grid container spacing={1} spacing={1} direction="column">
                ...
                <Grid item>
                  <Typography style={{ fontSize: '14px' }} onClick={() => contractEditMainRef.current.openModal()}>
                    Edit
                  </Typography>
                </Grid>
