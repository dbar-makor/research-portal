...
import { StyledTextField, StyledAutoComplete, OutlinedButton, FilledButton, AddButton, DeleteButton} from '../../styles/MainStyles';
import {  selectChosenResearch, changeChosenResearch } from "../../redux/researches/chosenResearchSlice";
import { useHistory, useLocation } from 'react-router';
import * as actionSnackBar from '../../redux/SnackBar/action';
import { validateLivePublication, validateEvent, validateEditedLivePublication } from "../Reusables/validationFunctions";
 ...
 console.log("chosenResearch", chosenResearch)
  ...
  useEffect(() => {
   if(chosenResearch){
    console.log("there's a chosenResearch");
     let coverImg = chosenResearch.attachments.find(attachment => attachment.file_type === "main_bg");
     let otherFiles = chosenResearch.attachments.filter(attachment => attachment.file_type !== "main_bg");
    //  let categoriesIDs = chosenResearch.categories.map(category => category.id)
     let editedLocalForm = {...chosenResearch, attachments: otherFiles};
  ...
  useEffect(() => {
    ...
    if(location.state?.from === "prearticle"){
      ...
      let coverImg = publication.attachments?.find(attachment => attachment.file_type === "main_bg");
      let otherFiles = publication.attachments?.filter(attachment => attachment.file_type !== "main_bg");
      // let categoriesIDs = publication.categories?.map(category => category.id)
      let editedLocalForm = {...publication, attachments: otherFiles};
...
console.log("contentNotOK.isText", contentNotOK.isText)
  ...
  const sendPublication = async (buttonMarker) => {
    ...
    let categoriesForServer = localCats.map(cat => cat.id)
    let tagsForServer =  localTags.map(tag => tag.id ? {id: tag.id} : {name:tag.name})
    ...
    if(buttonMarker === "done"){
      formToSend = {...formToSend, attachments: attachmentsCopy, categories: categoriesForServer, tags: tagsForServer,  description: description,status: "published", content: typeof formToSend.content === "string" ? JSON.parse(formToSend.content) : formToSend.content}
      console.log("formToSend", formToSend)
      
    }else if(buttonMarker === "save-draft"){
      formToSend = {...formToSend, attachments: attachmentsCopy, categories: categoriesForServer,  tags: tagsForServer, description: description, status: "draft", content: typeof formToSend.content === "string" ? JSON.parse(formToSend.content) : formToSend.content}
     

    }else if(buttonMarker === "preview"){
      formToSend = {...formToSend, attachments: attachmentsCopy, categories: localCats, tags: localTags, description: description , created_at : new Date()};
  ...
  const onDropCover = async(acceptedFiles) => {
    let coverImage = acceptedFiles[0]
    const formData = new FormData()
      formData.append('file', coverImage)
      try {
        const res = await axios.post(`${BASE_URL}${END_POINT.FILE}`, formData)
      if (res.status === 200) {
        const newCover = { file_type: 'main_bg', file_name: coverImage.name, file_name_system: res.data.file}
  ...
  return (
    <Grid container justifyContent='center'>
      <Grid item xs={10}>
        <Grid
            container
            className={classes.newArticleContainer}
          >
              ...
              <Grid container>
                  ...
                  <Grid item xs={12}>
                    <Grid container>
                      <Grid item xs={12}>
                        <MUIRichTextEditor
                        className={showEditorError ? `${classes.muiEditor} ${classes.editorError}` : `${classes.muiEditor}`}
                        ...
                        {...(chosenResearch && Object.keys(chosenResearch.content).length && {defaultValue: typeof chosenResearch.content !== "string" ? JSON.stringify(chosenResearch.content) : chosenResearch.content})}
                        {...(location.state?.from === "prearticle" && Object.keys(location.state?.publication.content).length && {defaultValue:  typeof location.state?.publication.content !== "string" ? JSON.stringify(location.state?.publication.content) : location.state.publication.content})}
                          controls={[
                            'bold',
                            'italic',
                            'underline',
                      ...
                      { contentNotOK.focus &&  contentNotOK.everTyped && !contentNotOK.isText && <Grid item xs={6} style={{color: "red", marginLeft: 8}}>
              ...
              <Grid item xs={12}>
                <Grid container>
                  ...
                  <Grid item xs={9}>
                   ...
                   />
                   { !coverImageOK.initial && <Typography variant="caption" className={classes.customError}>
                      This field is required
                    </Typography>}
                    <Grid container>
                            ...
                            <Grid item xs={5}>
                              <StyledTextField
                                onChange={(e) => {
                                  ...
                                  validateEvent({title:  e.target.value}, errorsEvent, setErrorsEvent,setValidationResultEvent )
                                }
                                }
                ...
                <Grid item xs={12}>
                    <Grid container justifyContent='space-between' className={classes.buttonsContainer}>
                       {( (chosenResearch && chosenResearch.status === "draft" ) || !chosenResearch) && <OutlinedButton onClick={() => sendPublication("save-draft")}>Save Draft</OutlinedButton>}
                        <OutlinedButton onClick={() => sendPublication("preview")}>Preview</OutlinedButton>
                        <FilledButton 
                          disabled={!validationResult || !validationResultEvent || !coverImageOK.final || !contentNotOK.isText} 
