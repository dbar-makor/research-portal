...
import { useStyles, StyledButton, StyledTextField, StyledCancelButton } from '../../../styles/PublicationsStyles';
...
function Comments(props) {
  ...
  const renderComment = (comment , index) => {
    return <Comment pubId={pubId} comment={comment} key={index} cmtNo={index === 0 ? comments.length : comments.length - index} replyHendler={(data) => addComment(data)} cmntId={comment.id} />;
    ...
    <Grid item xs={12}>
      ...
      <Grid container style={{ paddingTop: '25px' }}>
              ...
              <Grid item xs={12} style={{ paddingTop: 10 }}>
                <Grid container alignItems="flex-end" justifyContent="flex-end">
                  <Grid item style={{ marginRight: 10 }}>
                    <StyledCancelButton onClick={() => setOpenAddCmnt(!openAddCmnt)}>Cancel</StyledCancelButton>
                  </Grid>
                  <Grid item>
                    <StyledButton onClick={() => addComment(newComment)} disabled={newComment.content != '' ? false : true}>
                      Comment
                    </StyledButton>
                  </Grid>
                </Grid>
        ...
        <Grid item xs={12} align="center" >
          <Button style={{ color: '#1C67FF', textTransform: 'none', border: '1px solid #1C67FF', borderRadius: '21px', fontSize: '12px' }} onClick={() => loadMore()}>
            Load More
          </Button>
        </Grid>
      </Grid>
