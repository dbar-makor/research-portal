...
function DropZoneMulti(props) {
    ...
    const {onDrop, purpose = 'your attachments', setFileOK, fileTypes, localForm, deleteItem
        } = props
    const { getRootProps:getRootProps, getInputProps:getInputProps, isDragActive:isDragActive } = useDropzone({ onDrop, accept: fileTypes })
                ...
                <Grid container className={classes.attachmentsScrolledContainer}>
                        ...
                        {localForm.attachments && localForm.attachments.length
                            ? localForm.attachments.map((file, index) => (
                                ...
                                >
                                <Grid item xs={8}>
                                    <Typography>{file.file_name.length > 20 ? `${file.file_name.slice(0, 20)}...` : file.file_name}</Typography>
                                </Grid>
                                <Grid item xs={1}>
                                <DeleteButton
                                    disableRipple
