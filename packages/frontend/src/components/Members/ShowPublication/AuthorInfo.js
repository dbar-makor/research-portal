...
const AuthorInfo = (props) => {
  ...
  const [auther, setAuth] = useState({ name: 'Eliyahu Hillel', avatar: 'https://image.pngaaa.com/419/263419-middle.png' });
  ...
  return (
    <Grid item xs={12}>
      ...
      <Grid container direction="row" alignItems="center" style={{ paddingTop: 10 }} justifyContent="space-between">
        <Grid item xs={10}>
          <Grid container alignItems="center">
            <Grid item xs={1}>
              <Avatar alt="Remy Sharp" src={auther.avatar ? `${auther.avatar}` : 'https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260'} />
            </Grid>
            <Grid item xs={11}>
              <Grid container>
                <Grid item xs={12}>
                ...
                {lastDate ? (
                  <Grid item xs={12}>
                    <Typography className={classes.autherFont}>{`Updated: ${format(new Date(lastDate), 'dd MMM, yyyy hh:mm')}`}</Typography>
                  </Grid>
                ) : null}
              </Grid>
            </Grid>
