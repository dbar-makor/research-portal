...
Publication.createPublication = async (payload, result) => {
	try {
		...
		const publication_res = await db_helper.update(
			query.create_publication(processed_data),
			processed_data,
		);
		...
		if (processed_data.content) {
			...
			if (processed_data.type == 'live' && blocks) {
				...
				const update_statistics = await db_helper.update(
					query.update_statistics(res_statistics, user_id),
					res_statistics,
				);
		...
		for (category_uuid of categories) {
			...
			const notifiers_res = await db_helper.get(
				notification_query.get_users_by_category_id(category.id),
			);
			...
			const category_has_publication_res = await db_helper.update(
				query.add_category_has_publication(new_category_has_publication),
				new_category_has_publication,
			);
		...
		if (tags) {
			for (tag of tags) {
				...
				const publication_has_tag_res = await db_helper.update(
					query.add_publication_has_tag(new_publication_has_tag),
					new_publication_has_tag,
				);
		...
		if (events) {
			for (curr_event of events) {
				...
				const event_creation_res = await db_helper.update(
					query.create_event(event_to_save),
					event_to_save,
				);
		...
		if (attachments) {
			...
			for (const attachment of attachments) {
				...
				const res_save_in_db = await db_helper.update(
					query.create_attachments(attachment_data),
					attachment_data,
				);
		...
		const [publication_inserted] = await db_helper.get(
			query.get_publication_by_id(publication_res.insertId),
		);
...
Publication.getPublications = async (payload, result) => {
	...
	try {
		...
		const publications = await db_helper.get(
			query.get_publications_by_category_id(category_id, filter_by),
		);
...
Publication.updatePublication = async (payload, result) => {
	try {
		...
		const publication_res = await db_helper.update(
			query.update_publication(processed_data, publication_uuid),
			processed_data,
		);
		if (!publication_res.affectedRows) {
			return result({ status: 400 });
		...
		if (tags) {
			...
			const delete_publication_has_tag = await db_helper.update(
				query.delete_publication_has_tag(publication_id),
			);
		...
		if (events) {
			for (curr_event of events) {
				...
				if (event_uuid) {
					event_creation_res = await db_helper.update(
						query.update_event_by_uuid(event_uuid, event_to_save),
						event_to_save,
					);
				} else {
					event_creation_res = await db_helper.update(
	...
	try {
		...
		if (publication.type == 'live') {
			...
			if (publication.type == 'live' && blocks) {
				...
				if (publication.status) {
					res_statistics.published_total -= total_words;
					res_statistics.published_avg =
						res_statistics.published_total / res_statistics.posts
							? res_statistics.published_total / res_statistics.posts
							: 0;
				} else {
					res_statistics.draft_total -= total_words;
					res_statistics.draft_avg =
						res_statistics.draft_total / res_statistics.saved
							? res_statistics.published_total / res_statistics.posts
							: 0;
				}
				delete res_statistics.user_id;
...
Publication.socialPublication = async (payload, result) => {
	...
	try {
		...
		if (type === 'share') {
			...
			const [social] = await db_helper.get(
				query.get_social_by_publication_and_user_id(publication.id, user_id),
			);
			if (!social) {
				// Create new social
				const new_social = {
...
Publication.bookmarkPublication = async (payload, result) => {
	...
	try {
		...
		const [bookmark] = await db_helper.get(
			query.get_bookmark_by_publication_and_user_id(publication.id, user_id),
		);
		if (!bookmark) {
			// Create Bookmark
			const new_bookmark = {
...
Publication.getPublicationsOfUser = async (payload, result) => {
	...
	try {
		...
		for (let publication of publications) {
			const res_attachments = await db_helper.get(
				query.get_attachments_by_publication_id(publication.id),
			);
			const res_categories = await db_helper.get(query.get_categories_of_publication(publication.id));
			let format_publication = {};
			if (publication.type == 'live') {
