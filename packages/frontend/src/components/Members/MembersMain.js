...
function MembersMain() {
  ...
  const getPublications = async () => {
    try {
      let resp = await axios.get(`${BASE_URL}${END_POINT.PUBLICATION}/user`);
  ...
  const filterByCategory = (category) => {
    ...
    if (category.id !== '') {
      let filterPublications = publications.filter((p) => {
  ...
  useEffect(() => {
    ...
    if (search !== '') {
      let filterPublications = publications.filter((publication) =>
        //let categoriesNames = p.categories.map((category) => category.name)
          ...
          <Grid item xs={3}>
            <FormControl style={{ width: '100%' }} className={classes.selectFormControl}>
              ...
              <Select
                ...
                MenuProps={{ classes: { paper: classes.sortDropdownStyle, input: classes.input } }}
