    ...
    if (last_contracts.length > 0) {
      for (contract of last_contracts) {
        if (contract.status == 1) {
          ...
          const update_status = await db_helper.update(query.update_contract_status_to_false(contract), contract)
          if (!update_status) {
...
Contract.updateContract = async (payload, result) => {
  try {
    ...
    const res_contract = await db_helper.update(query.update_contract(processed_data, id), processed_data)
        ...
        if (val !== undefined || typeof val !== 'object') {
          switch (key) {
            ...
            case 'id':
              ...
              break
            case 'periodicity':
              if (val !== 'monthly' && val !== 'quarterly' && val !== 'half' && val !== 'fully') {
