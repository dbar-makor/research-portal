...
function AllInvoices() {
  ...
  const getAllInvoicesAsync = async () => {
    try {
      ...
      let resp = await axios.get(`${BASE_URL}${END_POINT.INVOICE}`, { params });
      if (resp.status === 200) {
                  ...
                  <Grid container justifyContent="flex-end">
                    ...
                    <Grid item xs={4} style={{ marginLeft: 15 }}>
                      <StyledTextField
                        ...
                        placeholder="Search"
                        InputProps={{
                          endAdornment: <SearchIcon onClick={(e) => inputHandler(e, 'INVOICE_ID')} style={{cursor: 'pointer' }} className={classes.searchIcon} />,
                        }}
                      />
                    </Grid>
                  </Grid>
