...
function SendContractView({ setStep, contractCopy, setContractCopy }) {
  ...
  const presentPDFContract = async () => {
    try {
      const res = await axios.get(`${BASE_URL}${END_POINT.CONTRACT}/pdf/${contractCopy.contract_id}`, { headers: { Accept: 'application/pdf' } });
      ...
      <Grid item xs={12}>
        <Grid container>
          ...
          <Grid item xs={12} className={classes.lowerSection}>
            <Typography className={classes.question}>Whom do you want to send the contract to for signature?</Typography>
            <Grid container className={classes.row}>
              ...
              <Grid container item xs={5} className={classes.btnWrapper}>
                <OutlinedButton className={classes.linkBtn} disabled={!validationResult.step1} onClick={sendEmail}>
                  {' '}
                  Send Link{' '}
                </OutlinedButton>
              </Grid>
