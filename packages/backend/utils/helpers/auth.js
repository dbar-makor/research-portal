...
const save_token_in_db = (token, user_id, location, device_type, token_type, code) => {
	return new Promise(async (resolve, reject) => {
		try {
			...
			const res_connection = await db_helper.update(query_auth.create_connection(), {
				user_id,
				device_type,
				location,
			});
			...
			const res_token = await db_helper.update(query_auth.create_token(), [
				res_connection.insertId,
				token_type,
				token,
				code,
			]);
...
const create_token = async (user, exp = '1d', token_type) => {
	return new Promise(async (resolve, reject) => {
		...
		jwt.sign(
			{ data: encrypted_and_encoded, algorithm: 'HS256', expiresIn: exp },
