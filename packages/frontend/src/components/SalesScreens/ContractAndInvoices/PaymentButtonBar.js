      ...
      <div className={classes.paymentBar} style={{ cursor: 'pointer' }} onClick={() => openInvoices()}>
        <div className={classes.progressBar}>
          ...
          <div style={{ backgroundColor: '#B8C3D8', width: '90px', height: '5px', borderRadius: 4 }}>
            <div style={{ height: '5px', width: `${precentage}%`, backgroundColor: '#1C67FF', borderRadius: 4 }}></div>
          </div>
        </div>
        {isOpen ? <ExpandLessIcon fontSize="small" style={{ color: '#1C67FF' }} /> : <ExpandMoreIcon fontSize="small" style={{ color: '#1C67FF' }} />}
      </div>
        ...
        <Grid container direction="column" spacing={2} style={{ paddingTop: 12 }}>
          ...
          <Grid item xs={11} style={{ marginLeft: '16px' }}>
            <Grid container spacing={1} direction="column">
              ...
              <Grid item>
                <Typography className={classes.popContent} onClick={() => contractEditMainRef.current.openModal()}>
                  Edit
                </Typography>
              </Grid>
              <Grid item>
