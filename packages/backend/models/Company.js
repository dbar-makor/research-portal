		...
		if (members) {
			for (member of members) {
				...
				for (category_uuid of categories) {
					...
					const res_category = await db_helper.update(
						query.create(user_has_category_object, 'user_has_category'),
						user_has_category_object,
					);
	...
	try {
		...
		if (res_company.client_id) {
			...
			for (member of res_members) {
				...
				const res_categories_id = await db_helper.get(
					query.get_full_categories_uuid_by_user_id(member_id),
				);
			...
			if (!res_company.prospect_id) {
				...
				const [res_sales_agent] = await db_helper.get(
					query.get_user_by_user_id(res_prospect.sales_id),
				);
			...
			const [res_current_constract] = await db_helper.get(
				query.get_current_contract_by_company_id(company_id),
			);
			if (res_current_constract) {
				const [res_salesman] = await db_helper.get(
					query.get_user_by_user_id(res_current_constract.sales_id),
				);
				...
				const [res_currency] = await db_helper.get(
					query.get_full_currency(res_current_constract.currency),
				);
			...
			if (res_last_contract) {
				const [res_salesman] = await db_helper.get(
					query.get_user_by_user_id(res_last_contract.sales_id),
				);
				...
				const [res_currency] = await db_helper.get(
					query.get_full_currency(res_last_contract.currency),
				);
...
Company.updateCompany = async (payload, result) => {
	try {
		...
		if (res_company.client_id) {
			const res_update_client = await db_helper.update(
				query.set_status_of_client(status, res_company.client_id),
				status,
			);
		} else {
			if (res_company.prospect_id) {
				const res_update_prospect = await db_helper.update(
					query.set_status_of_prospect(status, res_company.prospect_id),
					status,
				);
			}
		}
		...
		const res_update_company = await db_helper.update(
			query.update_company(processed_data, id),
			processed_data,
		);
...
Company.upgradeCompany = async (payload, result) => {
	try {
		...
		if (res_company.prospect_id) {
			const status = 0;
			const res_prospect = await db_helper.update(
				query.set_status_of_prospect(status, res_company.prospect_id),
				status,
			);
		}
		...
		const res_update_company = await db_helper.update(
			query.set_client_id_of_company(client_id, company_id),
			client_id,
		);
		if (!res_update_company.affectedRows) {
			return result({ status: 404 });
				...
				if (val !== undefined || typeof val !== 'object') {
					switch (key) {
						...
						case 'country':
							...
							break;
						case 'start_at':
							processed_payload.start_at = moment(val, 'YYYY-MM-DD').format(
								'YYYY-MM-DD hh:mm:ss',
							);
							break;
						case 'end_at':
							processed_payload.end_at = moment(val, 'YYYY-MM-DD').format(
								'YYYY-MM-DD hh:mm:ss',
							);
							break;
						case 'members':
