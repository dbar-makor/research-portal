  ...
  try {
    ...
    for (const ws of wss.clients) {
      ...
      if (notifiers.includes(session.user_id)) {
        const [res_settings_notifications] = await db_helper.get(settings_query.get_notifications_settings(session.user_id))
        ...
        if (res_settings_notifications.is_active && settings.publications.new_publications) {
          ...
          const res_insert_notification = await db_helper.update(query.insert_notification(notification), notification)
          if (!res_insert_notification) return result({ status: 500 })
          const [inserted_notification] = await db_helper.get(query.get_notification_by_id(res_insert_notification.insertId))
...
const update_notification = async (notification) => {
  try {
    const res_update_notification = await db_helper.update(query.update_notification(notification), notification)
  } catch (e) {
    console.log(e)
    return result({ status: 400 })
  }
...
module.exports = { send_notification, get_notifications, update_notification, set_is_read, set_new_notification, set_is_new, get_all_notifications, mark_all_read }
