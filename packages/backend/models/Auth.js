...
Auth.createAuth = async (payload, result) => {
  ...
  let is_2fa_active =
    process.env.IS_TWO_FACTOR_AUTH_ACTIVATED === "true" ? true : false;
  ...
  try {
    ...
    const [level_id_member] = await db_helper.get(
      query.get_level_id_by_name("member")
    );
    ...
    if (level_id_member.id == user.level_id) {
      ...
      const [comapny_res] = await db_helper.get(
        query.get_company_by_id(user.company_id)
      );

      // check status of if prospect or client
      if (comapny_res.client_id) {
        const [res_client] = await db_helper.get(
          query.get_client_by_id(comapny_res.client_id)
        );
        ...
        const [res_prospect] = await db_helper.get(
          query.get_prospect_by_id(comapny_res.prospect_id)
        );
    ...
    if (process.env.TWO_STEP_ACTIVE === "true") {
      ...
      if (type) {
        if (type === "mobile_app" && user.device_id) {
          return connection_with_web_notification_to_mobile(
            user,
            location,
            result
          );
        } else if (type === "email_confirm" && user.email) {
          return connection_with_web_confirmation_to_email(
            user,
            location,
            result
          );
        } else if (type === "sms_confirm" && user.phone) {
          return connection_with_app_confirmation_to_sms(
            user,
            location,
            result
          );
...
Auth.registerAuth = async (payload, result) => {
  try {
    ...
    const token = await auth_helper.create_token(
      user,
      process.env.TOKEN_EXP_PLATFORM,
      "platform"
    );
...
Auth.verifyTwofactorAuthMobileAnswer = async (payload, result) => {
  try {
    const { device_id, confirmation_number } =
      payload.twoFactorVerificationMobileAnswer;
    const { userIp: user_ip } = payload;
    ...
    const [user] = await db_helper.get(
      query.get_user_by_id(token_content.user_id)
    );
    const location = await global_helper.get_location(user_ip);
    const token = await auth_helper.create_token(
      user,
  ...
  try {
    ...
    if (email) {
      ...
      const message_bcc = process.env.AUTH_SEND_MAIL_BCC
        ? JSON.parse(process.env.AUTH_SEND_MAIL_BCC).filter(
            (email) => message_to != email
          )
        : [];
...
Auth.resetPassword = async (payload, result) => {
  try {
    ...
    if (
      ...
      !regex_helper.check_password(new_password)
    ) {
      return result({ status: 400 });
    }
    ...
    await db_helper.update(
      query.update_user_by_id(proccess_data, user.id),
      proccess_data
    );
    return result({ status: 200 });
  } catch (error) {
...
const connection_with_mobile = async (
  user,
  device_id = null,
  device_origin,
  location,
  ...
  await db_helper.update(
    query.update_device_id(user.id, device_id, device_origin),
    []
  );

  const token = await auth_helper.create_token(
    user,
    process.env.TOKEN_EXP_APP,
    "platform"
  );
  await auth_helper.save_token_in_db(
    token,
...
const connection_with_web_notification_to_mobile = async (
  user,
  location,
  result
) => {
  ...
  const verification_numbers = lodash.shuffle([
    temp_numbers[0],
    temp_numbers[1],
    temp_numbers[2]
  ]);
  ...
  if (user.device_os === "android") {
    ...
    const message = {
      ...
      data: {
        ...
        message:
          "Please type on the correct answer in order to register your account."
      },
...
const connection_with_web_confirmation_to_email = async (
  user,
  location,
  result
) => {
...
const connection_with_app_confirmation_to_sms = async (
  user,
  location,
  result
) => {
