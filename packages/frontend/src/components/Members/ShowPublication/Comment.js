...
import { useStyles, StyledCollape, StyledCancelButton, StyledButton } from '../../../styles/PublicationsStyles';
      ...
      <Grid container direction="column" style={{ paddingTop: 18 }}>
        ...
        <Grid item xs={12}>
          <Grid container alignItems="center">
            <Grid item>
              <Typography className={classes.commentWriter}>{`${comment.user.name} | ${format(new Date(comment.created_at ? comment.created_at : comment.date), 'dd.MM.yyy')}`}</Typography>
            </Grid>
            ...
            {comment.replies.length > 0 && (
              <Grid item style={{ marginLeft: 30 }}>
                <Typography className={classes.repliesNo} onClick={() => setOpen(!open)}>
                  {open ? <ArrowUp style={{ margin: '0px 5px 2px 0px' }} /> : <ArrowDown style={{ margin: '2px 5px 1px 0px' }} />}
          ...
          <Grid item>
            <Grid container>
              <Grid xs={12} style={{ paddingTop: 8 }}>
                <TextField variant="outlined" placeholder="Add a reply..." className={classes.textField} onChange={(e) => setReply({ ...reply, content: e.target.value })} value={reply.content} />
              </Grid>
              <Grid item xs={12} style={{ paddingTop: 10 }}>
                <Grid container alignItems="flex-end" justifyContent="flex-end">
                  <Grid item>
                    <StyledCancelButton style={{ marginRight: 10, width: 70 }} onClick={() => setOpenReply(false)}>
                      Cancel
                    </StyledCancelButton>
                  </Grid>
                  <Grid item>
                    <StyledButton disabled={reply.content != '' ? false : true} style={{ width: 70 }} onClick={() => repalyHendleChange(reply)}>
                      Reply
                    </StyledButton>
                  </Grid>
                </Grid>
